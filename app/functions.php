 ...
 * Usage:
 *
 * $ip = $_SERVER['REMOTE_ADDR']; // Or $_SESSION['user_id'] if logged in
 * $allowed = rateLimit($ip, 5, 60); // 5 attempts per 60 seconds
 *
 * if (!$allowed) {
 ...
 * }
 *
 * // Continue with form handling or login logic...
 *
 * @param string $identifier Can be IP address or user ID for signed in users
 * @param int $maxAttempts Maximum number of attempts
 * @param int $windowSeconds Range time of attempts (in seconds)
 * @return bool True if request is allowed, false if rate limited
 */
function rate_limit($identifier, $maxAttempts, $windowSeconds): bool {
    $dir = APP_PATH . 'limiter_data/';
    if (!file_exists($dir)) {
        mkdir($dir, 0755, true);
    }
    ...
    $data[] = $currentTime; // Record new attempt
    file_put_contents($file, json_encode($data));

    return true; // Allow access
}

function limit_requests($maxAttempts = 5, $windowSeconds = 60) {
    $allowed = rate_limit($_SERVER['REMOTE_ADDR'], $maxAttempts, $windowSeconds); // 5 attempts per 60 seconds
...
function logged_in(): array {
    ...
    if (empty($user) || $user['is_blocked']==1) {
        ...
        redirect($login_url, "Please sign in", "danger");
    }
...
function admin_logged_in(): array {
    $user = is_user_authenticated();
    // This function depends on is_user_authenticated() function
...
function header_view($name) {
    // Call page header/carousel
    require(APP_PATH . "views/landing/header/$name.header.php");
}

// PAGE VIEW FUNCTION
function page_view($name, $context=[]) {
...
// FUNCTION TO GENERATE IDS WITH PREFIX
function generateID($prefix = "TXN", $length = 9) {
    $id = '';
    for ($i = 0; $i < $length; $i++) {
        $id .= mt_rand(0, 9); // generates a random digit between 0 and 9
    }
    return $prefix . $id;
}
...
function fetch_image($image) {
    if ($image == null || !file_exists(APP_PATH . "media/$image")) {
        ...
        return STATIC_ROOT . "/dashboard/images/image_placeholder.png";
    } else {
...
function fetch_lga(int $lga_id): array {
    $matched_lgas = query_fetch("SELECT lga_id, lga_name, state_id FROM lga WHERE uniqueid = $lga_id LIMIT 1");

    if (!empty($matched_lgas)) {
        return $matched_lgas[0];
    }
...
function sendMail($to, $subject, $email_values = array()) {
    ...
    $email_values += [
        'site_name'=> ucwords($setting['name']),
    ...
    try {
        ...
        $mail->setFrom($setting['email'], ucwords($setting['name'])); // Update with your email and name
        $mail->addAddress($to);
...
function fetch_news($number_of_posts = 3) {
    
    try {
        $ch = curl_init();

        $url = "https://newsapi.org/v2/everything?q=cryptocurrency&apiKey=9ab989f35bb6450c8e2a717543c0c129";

        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'User-Agent: Asset-Trade/1.0'
        ));
        ...
        if ($e = curl_error($ch)) {
            ...
            // We only want n number of random articles per page load
            // Generate n random numbers within 0 to 99
            $random_numbers = array_rand(range(0, 99), $number_of_posts);
...
// FUNCTION TO UPDATE INVESTMENTS DAILY
function update_investment_daily(int $user_id) {
    ...
    foreach ($investments as $investment) {
        ...
        if ($today < $end_date && $today > $approved_date) {
            $interval = $investment['payout_interval']; // Daily
            $days_after_approval = ($today - $approved_date) / (60*60*24); // Convert from seconds to days
            ...
            if ($days_after_approval >= $interval) {
                ...
                if ($investment['returns'] !== $current_returns) {
                    ...
                    query_db($sql, ['returns'=>$current_returns, 'id'=>$investment['id']]);
                }
            }
        // Else if today is beyond end date of investment
            ...
            query_db("UPDATE investments SET returns = :returns, status = :status WHERE id = :id LIMIT 1", 
            [
            ...
            // Crediting user
            query_db("UPDATE users set balance = :balance WHERE id = :id LIMIT 1", 
            [
                'balance'=> $user['balance'] + $investment['roi'],
                'id'=> $user_id
            ]);
            // Notifying user
            query_db("INSERT INTO notifications (user_id, message, date) VALUES (:user_id, :message, :date)", 
            [
                'user_id'=> $user_id, 
                'message'=>"Congrats, your plan of $".$investment['amount']." has completed successfully and returned ROI", 
                'date' => $investment['end_date']
            ]);
...
// FUNCTION TO UPDATE INVESTMENTS HOURLY
function update_investment_hourly(int $user_id) {
    /* 
        Because we are working with hours, 
        the timezone is critical for accurate calculation 
    */

    // Fetching running investments
    $user = fetch_user($user_id);
    $investments = query_fetch("SELECT * FROM investments WHERE user_id = $user_id AND status = 'approved'");
    // Force strtotime() or date/time objects to parse in a given timezone
    date_default_timezone_set($user['timezone']);
    // Converting now to timestamp
    $now = new DateTimeImmutable('now', new DateTimeZone($user['timezone']));
    $current_time = $now->getTimestamp(); // Gives the exact current timestamp when the script runs


    foreach ($investments as $investment) {
        // Investment timestamps
        $approved_time = (new DateTimeImmutable($investment['approved_date'], new DateTimeZone($user['timezone'])))->getTimestamp();
        $end_time = (new DateTimeImmutable($investment['end_date'], new DateTimeZone($user['timezone'])))->getTimestamp();

        // Payout interval in hours — convert to seconds
        $interval_seconds = intval($investment['payout_interval']) * 3600;

        if ($current_time > $approved_time && $current_time < $end_time) {
            $elapsed_seconds = $current_time - $approved_time;

            if ($elapsed_seconds >= $interval_seconds) {
                // Full intervals since approval
                $intervals_passed = floor($elapsed_seconds / $interval_seconds);
                $calculated_returns = round($intervals_passed * $investment['profit_per_interval'], 2);

                if ($investment['returns'] != $calculated_returns) {
                    query_db("UPDATE investments SET returns = :returns WHERE id = :id LIMIT 1", [
        ...
        // Investment has ended but final ROI not paid
        elseif ($current_time >= $end_time && $investment['returns'] != $investment['roi']) {
            // Complete investment
            query_db("UPDATE investments SET returns = :returns, status = :status WHERE id = :id LIMIT 1", [
                'returns' => $investment['roi'],
                'status' => 'completed',
            ...
            query_db("INSERT INTO notifications (user_id, message, date) VALUES (:user_id, :message, :date)", [
                'user_id' => $user_id,
                'message' => "Congrats, your plan of $" . $investment['amount'] . " has completed successfully and your balance credited",
...
// FUNCTION TO FILTER PAYMENT METHOD
function available_payment_method(array $setting) {
    $payment_methods = [
        ['field' => "bitcoin_address", 'name' => "BITCOIN", "short_name" => "BTC", 'network' => "BTC", 'icon' => "bitcoin.png"],
        ['field' => "ethereum_address", 'name' => "ETHEREUM", "short_name" => "ETH", 'network' => "ERC-20", 'icon' => "ethereum.png"],
        ['field' => "litecoin_address", 'name' => "LITECOIN", "short_name" => "LTC", 'network' => "LTC", 'icon' => "litecoin.png"],
        ['field' => "dogecoin_address", 'name' => "DOGECOIN", "short_name" => "DOGE", 'network' => "DOGE", 'icon' => "dogecoin.png"],
        ['field' => "bitcoincash_address", 'name' => "BITCOIN CASH", "short_name" => "BCH", 'network' => "BCH", 'icon' => "bitcoincash.png"],
        ['field' => "usdt_address", 'name' => "USDT", "short_name" => "USDT", 'network' => "TRC-20", 'icon' => "usdt.png"],
        ['field' => "usdc_address", 'name' => "USDC", "short_name" => "USDC", 'network' => "ERC-20", 'icon' => "usdc.png"],
        ['field' => "perfect_money", 'name' => "PERFECT MONEY", "short_name" => "PM", 'network' => false, 'icon' => "perfect-money.png"],
        ['field' => "payeer", 'name' => "PAYEER", "short_name" => "PYR", 'network' => false, 'icon' => "payeer.png"]
    ];

    return array_filter($payment_methods, function($method) use ($setting) {
        $field = $method['field'];
        return isset($setting[$field]) && !empty($setting[$field]);
    });
}

// FUNCTION TO FETCH PAYMENT METHOD BY KEY
function get_payment_method_by_key(array $available, string $key) {
    $matched = array_filter($available, function($method) use ($key) {
        return isset($method['short_name']) && $method['short_name'] === strtoupper($key);
    });

    // Return the first match or null if not found
    return !empty($matched) ? array_values($matched)[0] : null;
}

// FUNCTION TO NOTIFY ADMIN
function notify_admin(string $admin_email, $subject, $message) {
    $email_values = [
        'name'=> "Admin", 
        'message'=> $message
    ];
    sendMail($admin_email, $subject, $email_values);
}